...
// Validate checks an API key against stored keys. Returns the tier if valid, or error.
func (m *Manager) Validate(ctx context.Context, fullKey string) (string, error) {
	var prefix string
	switch {
	...
	apiKey, err := m.store.GetAPIKeyByPrefix(ctx, keyPrefix)
	if err != nil {
...
func (m *Manager) RegenerateAll(ctx context.Context, tier string) (*GeneratedKey, error) {
	if err := m.store.DeleteAPIKeysByTier(ctx, tier); err != nil {
		return nil, fmt.Errorf("keys: delete old keys: %w", err)
	}
	return m.Generate(ctx, tier)
}

// EnsureKeys checks if keys exist for both tiers. If not, generates them.
// Returns (anonKey, serviceKey, error). nil means key already existed.
